...
import type { Cliente, ClientesListado } from '../../../../domain/finanzas/types';
...
import type { ClienteCreditosDto } from '../../../../domain/finanzas/clienteCreditos';

export const ClientesService = {
  getAll: async (params?: { page?: number; pageSize?: number; buscar?: string; zonaId?: string }): Promise<ClientesListado> => {
    const url = withQueryParams(API_ENDPOINTS_FINANZAS.CLIENTES, params ?? {});
  ...
  getCreditos: async (clienteId: string): Promise<ClienteCreditosDto> => {
    const url = withRouteParams(API_ENDPOINTS_FINANZAS.CLIENTE_CREDITOS, { id: clienteId });
    const response = await ApiService.get({ url });
  ...
  create: async (payload: Omit<Cliente, 'id'>): Promise<Cliente> => {
    const response = await ApiService.post({ url: API_ENDPOINTS_FINANZAS.CLIENTES, data: payload });
  ...
  update: async (id: string, payload: Omit<Cliente, 'id'>): Promise<Cliente> => {
    const url = withRouteParams(API_ENDPOINTS_FINANZAS.CLIENTE_BY_ID, { id });
